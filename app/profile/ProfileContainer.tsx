/**
 * Profile Container (Server Component)
 * Fetches user profile data on the server
 */

import { api } from '@/lib/api';
import { cookies } from 'next/headers';
import ProfileScene from './ProfileScene';

async function getUserProfile() {
  try {
    const cookieStore = await cookies();
    const accessToken = cookieStore.get('munshi_access_token')?.value;

    if (!accessToken) {
      return null;
    }

    // Prepare headers for server-side API calls
    const allCookies = cookieStore.getAll();
    const cookieHeader = allCookies.map(c => `${c.name}=${c.value}`).join('; ');
    const config = { headers: { Cookie: cookieHeader } };

    // Fetch user profile
    const user = await api.user.getProfile(config);

    return { user };
  } catch (error) {
    console.error('Profile data fetch error:', error);
    return null;
  }
}

export default async function ProfileContainer() {
  const data = await getUserProfile();

  if (!data) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <p>Loading profile...</p>
      </div>
    );
  }

  return <ProfileScene initialData={data} />;
}
